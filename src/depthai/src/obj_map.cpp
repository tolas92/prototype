...
class PersonPose : public rclcpp::Node
{
public:
  PersonPose()
  : Node("Person_Pose")
  {
    ...
    point_sub_.subscribe(this, "/spatial_pose");
    tf2_filter_ = std::make_shared<tf2_ros::MessageFilter<geometry_msgs::msg::PoseStamped>>(
      point_sub_, *tf2_buffer_, target_frame_, 100, this->get_node_logging_interface(),
      this->get_node_clock_interface(), buffer_timeout);
    // Register a callback with tf2_ros::MessageFilter to be called when transforms are available
    tf2_filter_->registerCallback(&PersonPose::msgCallback, this);

    publisher_ = this->create_publisher<geometry_msgs::msg::PoseStamped>("person_pose", 10);
...
int main(int argc, char * argv[])
{
  ...
  rclcpp::spin(std::make_shared<PersonPose>());
  rclcpp::shutdown();
  return 0;
}